...
wine_v4_set1 <- wine_v4 %>% filter(X<=40000)

lat_lon_dfinput <- wine_v4_set1 %>% mutate(id=X) %>% select(id, latitude, longitude)
...
wine_v5_set1 <- wine_v4_set1 %>% 
  mutate(stationid = as.character(stationids[,1])) %>%
  select(X, stationid)

save(wine_v5_set1, file="wine_v5_set1.RData")
