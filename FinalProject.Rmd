...
wine_v4$min <- as.Date(paste(wine_v4$year, minmonth, "01", sep="-"), "%Y-%m-%d")
maxmonth <- ifelse(wine_v4$latitude>0,"10","04")
maxday <- ifelse(wine_v4$latitude>0, "31", "30")
wine_v4$max <- as.Date(paste(wine_v4$year, maxmonth, maxday ,sep="-"), "%Y-%m-%d")
...
empty = matrix(rep(999,2), nrow = 1)
f = sapply(1:nrow(wine_v4), function(x) {  
  tryCatch(meteo_tidy_ghcnd(as.character(wine_v4$stationid[x]), 
                            keep_flags = FALSE,                     
                            var = c("prcp","tavg"), 
                            date_min = as.character(wine_v4$min[x]),                     
                            date_max = as.character(wine_v4$max[x])) %>%  summarize(avgprcp=mean(prcp/10, na.rm=TRUE),                                                                            avgtemp=mean(tavg/10, na.rm=TRUE)), error = function(e) {empty})  
  }) %>% t() %>% as.data.frame()
```
