...
wine_v4 <- wine_v3 %>% filter(is.na(longitude) == FALSE)
head(wine_v4, n=100)
lat_lon_df <- wine_v4 %>% mutate(id=X) %>% select(id, latitude, longitude)
head(lat_lon_df)
x = lat_lon_df[1,]

nearby_station <- NULL
for(i in 1:20){
  lat_lon_df2 <- lat_lon_df[i,]
  nearby_station1 <- data.frame(meteo_nearby_stations(lat_lon_df[i,] = lat_lon_df[i,], station_data = station_data, limit=1))
  nearby_station <- rbind(nearby_station,nearby_station1)
}

nearby_station <- meteo_nearby_stations(lat_lon_df = lat_lon_df, station_data = station_data, limit=1)
left_join(wine_v4,nearby_station, by="id")
View(nearby_station)
...
minmonth <- ifelse(wine_v4$latitude>0,"08","02")
min <- as.Date(paste(wine_v4$year, minmonth, "01", sep="-"), "%Y-%m-%d")
maxmonth <- ifelse(wine_v4$latitude>0,"10","04")
maxday <- ifelse(wine_v4$latitude>0, "31", "30")
max <- as.Date(paste(wine_v4$year, maxmonth, maxday ,sep="-"), "%Y-%m-%d")

meteo_tidy_ghcnd(nearby_station$stationid, keep_flags = FALSE, var = c("prcp","tavg"), date_min = min, date_max = max)


weather <- function(x){
  df <- data.frame(id=wine_v3$region[x,], latitude=wine_v3$latitude[x,], longitude=wine_v3$longitude)
  nearby_station <- meteo_nearby_stations(lat_lon_df = lat_lon_df, station_data = station_data, limit=1)
  meteo_tidy_ghcnd(nearby_station$stationid, keep_flags = FALSE, var = c("prcp","tavg"), date_min = min, date_max = max) %>%
  summarize(avgtemp = mean("tavg"), totrain = sum("prcp"))
}

weather(wine_v3$latitude, wine_v3$longitude, wine$min, wine$max)
...
weather <- meteo_tidy_ghcnd(stationid ="SPE00120620", keep_flags = FALSE, var = c("prcp","tavg"), date_min = lat_lon_df$min, date_max = lat_lon_df$max)
View(weather)

#sqrt(toy)
lat_lon_df <- data.frame(id = c("lakeMichigan"),latitude = c(43.45), longitude = c(-87.22))
nearby_stations <- data.frame(meteo_nearby_stations(lat_lon_df = lat_lon_df, station_data = station_data, limit=1))

View(nearby_stations)
class(nearby_stations)
as.data.frame(nearby_stations)
weather <- meteo_tidy_ghcnd(stationid ="SPE00120620", keep_flags = FALSE, var = c("prcp","tavg"), date_min = lat_lon_df$min, date_max = lat_lon_df$max)
View(weather)
