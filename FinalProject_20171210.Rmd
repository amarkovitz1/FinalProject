...
Data visualizations include average ratings by type of wine (using boxplots), average rating by price (correlation plot), average ratings by country (using maps), average ratings by country and year (using the tile plots in R), and average ratings by type of wine and year (using the tile plots in R). NOTE (when we have weather data, can also add in correlation plots between prec./temp and rating)
...
##Mean Prices (not shown in results)
##across the world
mapCountryData(sPDF,
...
##step 1: split into a testing and training dataset
library(caret)
set.seed(658)
...
#http://theanalyticalminds.blogspot.com/2015/04/part-4a-modelling-predicting-amount-of.html



##simple code (start with few trees)
fit <- randomForest(points ~ price + longitude + latitude + as.factor(variety) + as.factor(winery) + as.numeric(year), #add precip and temp
                      data=wine_v4, 
                      importance=TRUE, 
                      ntree=500)

##plot fit to see how it improves with each tree (how many trees should I make?)
plot(fit)
...
##Plot the actual vs predicted values in the 
