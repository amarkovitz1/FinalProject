...
wine_v4_newset1 <- wine_v4 %>% filter(X<=65000)

lat_lon_dfinput <- wine_v4_newset1 %>% mutate(id=X) %>% select(id, latitude, longitude)
...
wine_v5_newset1 <- wine_v4_newset1 %>% 
  mutate(stationid = as.character(stationids[1,])) %>%
  select(X, stationid)

save(wine_v5_newset1, file="wine_v5_newset1.RData")

##inner join back to the wine_v4

wine_v6_newset1 <- inner_join(wine_v4, wine_v5_newset1, by="X")
...
minmonth <- ifelse(wine_v6_newset1$latitude>0,"08","02")
wine_v6_newset1$min <- as.Date(paste(wine_v6_newset1$year, minmonth, "01", sep="-"), "%Y-%m-%d")
maxmonth <- ifelse(wine_v6_newset1$latitude>0,"10","04")
maxday <- ifelse(wine_v6_newset1$latitude>0, "31", "30")
wine_v6_newset1$max <- as.Date(paste(wine_v6_newset1$year, maxmonth, maxday ,sep="-"), "%Y-%m-%d")


#precip is in tenths of mm, tempavg is in tenths of degrees C

empty = matrix(rep(999,2), nrow = 1)
f = pbsapply(1:nrow(wine_v6_newset1), function(x) {
  tryCatch(meteo_tidy_ghcnd(as.character(wine_v6_newset1$stationid[x]), 
                     keep_flags = FALSE, 
                     var = c("prcp","tavg"), 
                     date_min = as.character(wine_v6_newset1$min[x]), 
                     date_max = as.character(wine_v6_newset1$max[x])) %>% 
    # group_by(id) %>%
    summarize(avgprcp=mean(prcp/10, na.rm=TRUE),
            avgtemp=mean(tavg/10, na.rm=TRUE)), error = function(e) {empty})
  }) %>% t() %>% as.data.frame()

wine_v7_newset1 <- wine_v6_newset1 %>% 
  mutate(avgprcp = f$V1, avgtemp = f$V2)

View(wine_v7_newset1)
save(wine_v7_newset1, file="wine_v7_newset1.RData")
